...
import { build, mailToken, mockResetTokenDeletionFailure, nodemailerMock, password, verifiedAdmin, verifiedUser } from '#test/helper.js';
...
test('Better Auth administration', async (t) => {
  ...
  await t.test('reset-token invalidation failure prevents an admin password change', async () => {
    ...
    try {
      assert.equal((await app.inject().post('/api/auth/admin/set-user-password')
        .headers({ origin: process.env.BASE_URL, ...admin.headers })
        .payload({ userId: member.user.id, newPassword: `${password}Changed` })).statusCode, 500);
